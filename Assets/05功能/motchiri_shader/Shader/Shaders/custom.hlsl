...
#define LIL_V2F_FORCE_TEXCOORD2
...
#define LIL_CUSTOM_VERTEX_OS \
	ParametersCompression();\
	float contactStrength = MaxContactStrength(positionOS.xyz,input.normalOS);\
	float3 offset = ContactStrengthToOffset(contactStrength,input.normalOS,float4(input.uv0,0,0));\
	if(_cancel==1){contactStrength=0;offset=float3(0,0,0);}\
	positionOS.xyz += offset;\
	if(_normal==1){input.normalOS = CalcNewNormal(positionOS.xyz,offset,input.normalOS,input.tangentOS,float4(input.uv0,0,0));}\
	input.uv2.x = tex2Dlod(_Mask,float4(input.uv0,0,0)).r*contactStrength;\
...
#define BEFORE_OUTPUT \
	fd.col *= lerp(1,_color,saturate(1/(1+exp(-(1-_blur)*40*((_color.a*input.uv23.x)-(1-_ao)*1.3)))));\
