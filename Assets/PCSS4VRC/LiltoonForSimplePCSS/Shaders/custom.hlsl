...
#ifdef SPOT
#include "Assets/nHaruka/PCSS4VRC/PCSSLogic/AutoLight.cginc"
#endif
uniform float _ShadowNormalBias = 0.003;
uniform float _UseMaskTex = 0;
#define unity_LightShadowBias float3(0,0,_ShadowNormalBias)

/*
#define NGSS_GLOBAL_OPACITY_DEFINED
#define NGSS_BIAS_FADE 0.015
#define NGSS_BIAS_SCALE 1.0			
...
//uniform float4 _LightColor0 = float4(0, 0, 0,0);
...
#define LIL_CUSTOM_PROPERTIES 
/*
uniform int NGSS_TEST_SAMPLERS = 32; 
uniform int NGSS_FILTER_SAMPLERS = 64; 
uniform float NGSS_LOCAL_SAMPLING_DISTANCE = 40; 
uniform float NGSS_GLOBAL_OPACITY = 0.0; 
uniform float NGSS_PCSS_FILTER_LOCAL_MIN = 0.01; 			//Close to blocker (If 0.0 == Hard Shadows). This value cannot be higher than NGSS_PCSS_FILTER_LOCAL_MAX
uniform float NGSS_PCSS_FILTER_LOCAL_MAX = 1.0; 
uniform float NGSS_FORCE_HARD_SHADOWS = 0.0; 
uniform float NGSS_PCSS_LOCAL_BLOCKER_BIAS = 0.0005;
#define NGSS_GLOBAL_OPACITY_DEFINED
...
#if defined(SPOT) && defined(NGSS)  
	#define BEFORE_SHADOW \
		fd.lightColor = step(_LightColor0.rgb , 0.1f) * fd.lightColor.rgb * 100;\
		fd.indLightColor = step(_LightColor0.rgb , 0.1f) * fd.indLightColor.rgb * 100;\
		fd.addLightColor = step(_LightColor0.rgb , 0.1f) * fd.addLightColor.rgb * 100;
#elif  defined(NGSS)
	#define BEFORE_SHADOW \
		fd.lightColor = fd.lightColor.rgb * _EnvLightStrength;\
		fd.indLightColor = fd.indLightColor.rgb * _EnvLightStrength;\
		fd.addLightColor = fd.addLightColor.rgb * _EnvLightStrength;
#endif
