#ifndef LIL_TESSELLATION_INCLUDED1
#define LIL_TESSELLATION_INCLUDED1
...
lilTessellationFactors hullConst(InputPatch<appdataCopy, 3> input)
{
    ...
    // メッシュのフチ以外でもテッセレーションが適用されるように変更
    ...
    tessFactor = (pos[0].x >  1.2 && pos[1].x >  1.2 && pos[2].x >  1.2) ||
                 (pos[0].x < -1.2 && pos[1].x < -1.2 && pos[2].x < -1.2) ||
                 (pos[0].y >  1.2 && pos[1].y >  1.2 && pos[2].y >  1.2) ||
                 (pos[0].y < -1.2 && pos[1].y < -1.2 && pos[2].y < -1.2) ? 0.0 : tessFactor;
